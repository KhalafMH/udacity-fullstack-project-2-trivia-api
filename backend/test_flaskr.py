...
class TriviaTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with self.app.app_context():
            ...
            self.db.engine.execute("""
                ...
                VALUES (1, 'Science'), (2, 'Sport'), (3, 'History');
            """)
            self.db.engine.execute("""
                ...
                INSERT INTO questions (question, answer, difficulty, category) 
                VALUES ('When did the big bang happen', 
                        'When the universe was created', 3, 1);
    ...
    def test_get_questions_returns_correct_result(self):
        ...
        try:
            exists = next(q for q in result.json['questions'] if
                          all(item in q.items() for item in question.items()))
    ...
    def test_delete_question_succeeds(self):
        ...
        self.assertDictEqual(
            {"success": True, "deleted_question_id": question_id}, result.json)
        with self.app.app_context():
            value = self.db.engine.execute(
                f"SELECT * FROM questions WHERE id={question_id}").first()
    ...
    def test_search_questions_returns_expected_results(self):
        ...
        result1 = client.post("/api/search/questions",
                              json={"searchTerm": "the"})
        self.assertEqual(200, result1.status_code)
        self.assertEqual(2, len(result1.json["questions"]))

        result2 = client.post("/api/search/questions",
                              json={"searchTerm": "bang"})
        self.assertEqual(200, result2.status_code)
        self.assertEqual(1, len(result2.json["questions"]))
        self.assertEqual("When did the big bang happen",
                         result2.json["questions"][0]["question"])

        result3 = client.post("/api/search/questions",
                              json={"searchTerm": "BANG"})
        self.assertEqual(200, result3.status_code)
        self.assertEqual(1, len(result3.json["questions"]))
        self.assertEqual("When did the big bang happen",
                         result3.json["questions"][0]["question"])

        result4 = client.post("/api/search/questions",
                              json={"searchTerm": "NonexistentTerm"})
    ...
    def test_get_questions_by_category_returns_expected_questions(self):
        ...
        self.assertEqual(2, len(result1.json['questions']))
        all(self.assertEqual('Science', question['category']) for question in
            result1.json['questions'])
    ...
    def test_get_questions_by_category_fails_when_category_is_given_by_name(
            self):
        client = self.client()
        category = 'Science'
    ...
    def test_get_questions_by_category_fails_when_passed_a_nonexistent_category_id(
            self):
        client = self.client()
        category_id = 1000
    ...
    def test_get_quiz_question_returns_a_question_in_the_specified_category(
            self):
        client = self.client()
